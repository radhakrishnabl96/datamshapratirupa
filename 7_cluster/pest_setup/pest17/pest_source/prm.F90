...
16         format('Subroutine PRM_SAVETEST must not be called before data has been ', &
           'provided for all agents through subroutine PRM_SLAVEDAT. This has not be ', &
           'done for agent number ',a,'.')
           go to 9890
         end if
       end do
...
20     format(/,' AGENT DETAILS:-',/)
       write(prm_mr,50)
50     format(' Agent Name',t40,'PAGENT Working Directory')
       write(prm_mr,55)
55    format(' ----------',t40,'------------------------')
       do islave=1,nslave
...
630          format(/,' Only ',a,' agents are alive.')
             write(6,640)
             write(prm_mr,640)
640          format(/,' Parallel processing will commence using only these agents.',/' Others will be used ',   &
             ...
             write(prm_mr,643)
643          format(' No agents are alive.')
#ifdef FLUSHFILE
              call flush(prm_mr)
#endif
             write(amessage,644)
644          format('No agents are alive. Start at least one agent and then restart program.')
             go to 9890
           end if
         end if
...
621    continue
       ...
       write(prm_mr,1100)
1100   format(/,/,' AGENT MODEL INPUT AND OUTPUT FILES:-')
       do islave=1,nslave
         write(prm_mr,1120) trim(aslave(islave))
1120     format(/,' Agent "',a,'" ----->')
         write(prm_mr,1130) trim(aslave(islave))
1130     format(/,'     Model input files on agent "',a,'":-')
         do i=1,ninfle
...
1180     format(/,'     Model output files on agent "',a,'":-')
         do i=1,noutfl
...
1250     format(/,'     Model command line for agent "',a,'":-')
...
750          format(3x,a,':- agent "',a,'" has just been detected.')
#ifdef FLUSHFILE
             call flush(prm_mr)
#endif
             write(6,751) trim(aslave(islave))
751          format(/,' Agent "',a,'" has just been detected.')
             idetect=1
             idet(islave)=1
             afile=trim(asldir(islave))//mreadfle
             call prm_open_new_message_file(ierr,prm_mf,afile)
...
753          format(3x,a,':- agent "',a,'" has just reappeared.')
#ifdef FLUSHFILE
             call flush(prm_mr)
#endif
             write(6,754) trim(aslave(islave))
754          format(/,' Agent "',a,'" has just reappeared.')
             idetect=1
             istats(islave)=0
           end if
	 end if
...
220	     format(3x,a,':- agent "',a,'" finished execution; old run so results not needed.')
#ifdef FLUSHFILE
             call flush(prm_mr)
#endif
             go to 250
...
230          format(3x,a,':- agent "',a,'" finished execution; same run already completed by another agent.')
#ifdef FLUSHFILE
             call flush(prm_mr)
#endif
             go to 250
...
240        format(3x,a,':- agent "',a,'" finished execution; reading results.')
#ifdef FLUSHFILE
           call flush(prm_mr)
#endif
           ncall=ncall+1
...
2491           format(3x,a,':- model run failure on agent "',a,'". Failure is ignored.')
#ifdef FLUSHFILE
               call flush(prm_mr)
#endif
               go to 2471
...
251	         format(/,' Problem encountered in reading results of run on agent "',  &
                 a,'".',/,' Will carry out run again.',/)
                 iproblem=1
                 write(prm_mr,249,err=9900) result,trim(aslave(islave))
249              format(3x,a,':- problems continuing with run on agent "',a,'". Will run model again.')
#ifdef FLUSHFILE
                 call flush(prm_mr)
#endif
                 go to 50
...
247          format(3x,a,':- problem (either model or communications) in reading results from agent "',a,'".')
#ifdef FLUSHFILE
             call flush(prm_mr)
#endif
             do ii=1,6
...
1070       format(3x,a,':- agent "',a,'" commencing model run.')
#ifdef FLUSHFILE
           call flush(prm_mr)
#endif
...
1230     format(3x,a,':- agent "',a,'" commencing model run because it can ',   &
...
1520   format(3x,a,':- agent "',a,'" commencing model run because agent "',a,'" is overdue.')
#ifdef FLUSHFILE
       call flush(prm_mr)
#endif
       call mio_write_model_input_files(jfail,npar,apar,parreg(1,irun),asldir(islave))
...
9210   format(3x,a,':- cannot write to agent "',a,'" to commence model run.')
#ifdef FLUSHFILE
       call flush(prm_mr)
#endif
       istats(islave)=150
...
9300   call prm_getsecs(istrtme(islave),0)
       write(prm_mr,9310) result,trim(aslave(islave))
9310   format(3x,a,':- previous observ.rdy file on agent "',a,'" not deleted.')
#ifdef FLUSHFILE
       call flush(prm_mr)
#endif
       istats(islave)=150
...
9000    ifail=1
        write(amessage,9010)
9010    format('Could not send message to agents to stop them.')
        return
