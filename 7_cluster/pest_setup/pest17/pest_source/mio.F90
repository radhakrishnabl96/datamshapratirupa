...
! -- Filenames
      ...
      character (len=3000)               :: dline        ! Character string for text storage
...
261       format(a,' cannot read observation "',a,'" from line ',a,     &
          ...
          if(instruction(n2:n2).ne.'!') then
            write(amessage,262) trim(errsub)
262         format(a,' instruction missing "!" symbol.')
            go to 9995
          end if
          almark=0
